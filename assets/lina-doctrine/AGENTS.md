...
Destino em runtime: <workspace>/AGENTS.md  (Codex = tier 2)

ADAPTAÇÃO TIER 2 (vs CLAUDE.md): o CONTEÚDO dos 8 blocos é IDÊNTICO ao CLAUDE.md.
O que muda é só o mecanismo de bootstrap:
  • Codex NÃO tem hook `SessionStart` nativo → esta doutrina (AGENTS.md) é o
    canal ÚNICO de bootstrap, à prova de falhas. Não dependa de injeção de input.
  • O app pode RE-INJETAR um lembrete curto (papel + colegas + vault) após N
    turnos, quando detectar que o contexto provavelmente rolou.
  • Skills de papel entram como SEÇÃO neste arquivo (não em .claude/skills/);
    a skill de A2A (`lina-agent-bus`) é descrita aqui e reforçada pelos verbos `lina`.
