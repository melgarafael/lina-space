...
## Portas registradas (deferidas — NÃO são gate de saída F4-WA)

- **WA2B-2 (CLI `add`/`rm`)** — design de Terminal K (não implementar até abrir, pós-gate, com o fundador):
  - **`add`** (`lina webhook add --node @X --instruction "..." [--level ...]`): gesto `webhook.add` na fila de broker (molde do `webhook.test`), payload `{node,instruction,level}` → app drena → `configure_hook_active` → `HookConfig{hook_id,secret}`. **O secret NUNCA volta ao CLI** — o `lina` é rodado TAMBÉM por terminais de AGENTE; secret em qualquer PTY = agente captura = fura ADR 0004. Aparece SÓ na tela do app (reusa `push_webhook_outcome`/`present_outcome`, 1×, superfície humano-exclusiva). O CLI recebe SÓ a URL (não-secreta, já inclui a porta viva — bônus: resolve a porta-não-publicada) via reply do `--await`, e imprime "criado, endereço X; a senha apareceu na tela do app". Rejeitadas (furam a doutrina): FIFO/named-pipe (secret no terminal → agente captura); reply via outbox (toca disco).
  - **`rm`** (`lina webhook rm <id>`): gesto `webhook.rm` (broker, `ref=webhook:<id>`) → app dropa o binding vivo + apenda **`WebhookRemoved { hook_id, #[serde(default)] reason }`** (aditivo, META no `apply`, `kind()` arm, sem campo de autoridade — origem server-side). Reversível (re-add) → gesto humano (HUMAN_GESTURE), não custódia pesada. Replay: `replay_configured` processa Configured/Removed por ordem de `seq` (hook removido sai do rebound); `project_webhooks` (list/show) idem.
- **Fiação app-level do gate (WA4)** — porta **F4-1** (junto de `F41-BROKER-APP`): propagar `MsgOrigin::System` até o `run_do`/broker no caminho de execução de ação externa real → marcador `WebhookActionGated` no fluxo real + `effective_level` em gated-soft. O backstop universal (`run_custody` gateia toda ação custodiada) já garante 0-exec sem gate; a fiação é o marcador/auditoria, e precisa do caminho de ação externa real (stub até F4-1).
