...
4. **Call-site de LEITURA (anti-órfã — alerta do Terminal K):** `channel_read::ler_conversa` (core, pronto) tem borda genérica `puxar: FnOnce() -> Result<Vec<M>, E>` e **NÃO importa `channel_waha` de propósito** (fronteira). O **adaptador de 1 linha** `|| waha_client.read(api_key, chat_id, limit)` (assinatura final confirmada B↔K: `read(&self, api_key, chat_id, limit) -> Result<Vec<WahaMessage>, WahaError>`) mora no **call-site no app** (`bridge.rs`, camada briefing/leitura) — também OFF-CRITICAL-PATH. **Sem esse call-site explícito, `ler_conversa` fica exposto sem chamador real (órfão funcional).** A próxima sessão DEVE atribuí-lo.

**RASCUNHO existente a APROVEITAR (não começar do zero):** `app/lina-gpui/src/whatsapp_modal.rs` (589 linhas, untracked) — o Especialista adiantou a CASCA do modal: núcleo PURO testável (`WhatsAppModal`: fases form/pairing, `ConnectPlan`, foco/validação) + render gpui + `WaPhase::AwaitingScan{qr_png}` desenhado pela thread off-critical-path. ⚠️ **Está ÓRFÃO** — sem `mod whatsapp_modal;` no `main.rs`, sem caller, NÃO compilado (por isso o build passa ignorando-o). A próxima sessão: **revisar o rascunho → declarar `mod` → fiar** (o caller que roda `WahaClient::connect/qr` na thread e deposita o `ChannelConnectState`; `A::ConnectChannel` no `main.rs:2193` ainda abre o modal de credencial F4-0, precisa apontar pro `WhatsAppModal`).
