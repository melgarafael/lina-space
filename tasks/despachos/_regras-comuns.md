...
4. **⚠️ BUG CONHECIDO DE IDENTIDADE (em fix nesta rodada):** `lina whoami`/`handshake` podem dizer que você é "Automatizador" — IGNORE; sua identidade é a do seu despacho. **NÃO use verbos `lina`** (ask/handoff/broadcast/status) nesta rodada — o carimbo de remetente está trocado. Reporte SÓ pelo arquivo de entrega.
