...
| **F1-4** | `WorkspaceCreated{workspace_id, default_cwd}` (diretório de trabalho escolhido na criação — M9) · `WorkspaceDefaultCwdSet{workspace_id, path}` (editar depois; F1-4-1 critério 6) · `WorkspaceClosed{workspace_id}` · `WorkspaceSwitched{from, to}` · `WorkspaceRestored{workspace_id, nodes}` · `LicenseChecked{tier, workspace_limit, outcome}` | `default_cwd` é o cwd PADRÃO de todo novo terminal/agente do Espaço (fundador 2026-06-09); novos nós resolvem cwd = `default_cwd` do Espaço ativo (senão fallback `n-<uuid>` isolado — interação ADR 0022). `LicenseChecked` NUNCA loga chave/assinatura (material sensível fora do log). Com o ADR 0010, eventos de roster passam a carregar `workspace_id` — campo aditivo `serde(default)` (workspaces antigos = workspace único default; replay antigo intacto). |
